# Muffin Dev for Unity - Core - Extensions

Contains extensions for native types or UnityEngine library classes.

## Native types extensions

- [`Array` extensions](./array.md)
- [`int` extensions](./int.md)
- [`string` extensions](./string.md)
- [`Type` extensions](./type.md)

## Unity types extensions

- [`AnimationCurve` extensions](./animation-curve.md)
- [`Camera` extensions](./camera.md)
- [`Color` extensions](./color.md)
- [`Component` extensions](./component.md)
- [`GameObject` extensions](./game-object.md)
- [`Gradient` extensions](./gradient.md)
- [`GUIStyle` extensions](./gui-style.md)
- [`Input` extensions](./input.md)
- [`List` extensions](./list.md)
- [`Object` extensions](./object.md)
- [`Rect` extensions](./rect.md)
- [`ScriptableObject` extensions](./scriptable-object.md)
- [`Transform` extensions](./scriptable-object.md)
- [`Vector2Int` extensions](./vector2-int.md)